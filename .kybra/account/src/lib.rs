...
async fn get_account(_cdk_user_defined_Id: candid::Int) -> String {
    unsafe {
        ...
        let method_py_object_ref =
            _kybra_unwrap_rust_python_result(_kybra_scope.globals.get_item("get_account", vm), vm);
...
#[ic_cdk_macros::query()]
#[candid::candid_method(query)]
async fn get_balance(_cdk_user_defined_Id: candid::Int) -> candid::Int {
    unsafe {
        let _kybra_interpreter = _KYBRA_INTERPRETER_OPTION.as_mut().unwrap();
        let _kybra_scope = _KYBRA_SCOPE_OPTION.as_mut().unwrap();
        let vm = &_kybra_interpreter.vm;
        let method_py_object_ref =
            _kybra_unwrap_rust_python_result(_kybra_scope.globals.get_item("get_balance", vm), vm);
        let invoke_result = vm.invoke(
            &method_py_object_ref,
            (_cdk_user_defined_Id.try_into_vm_value(vm).unwrap(),),
        );
...
async fn get_all_accounts() -> String {
    unsafe {
        ...
        let method_py_object_ref = _kybra_unwrap_rust_python_result(
            _kybra_scope.globals.get_item("get_all_accounts", vm),
            vm,
        );
        let invoke_result = vm.invoke(&method_py_object_ref, ());
        match invoke_result {
            Ok(py_object_ref) => {
                let _kybra_final_return_value =
                    _kybra_async_result_handler(vm, &py_object_ref, vm.ctx.none()).await;
...
async fn create_account(
    ...
    unsafe {
        ...
        let vm = &_kybra_interpreter.vm;
        let method_py_object_ref = _kybra_unwrap_rust_python_result(
            _kybra_scope.globals.get_item("create_account", vm),
            vm,
        );
        let invoke_result = vm.invoke(
            &method_py_object_ref,
