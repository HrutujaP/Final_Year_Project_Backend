...
async fn get_message() -> String {
    unsafe {
        ...
        let method_py_object_ref =
            _kybra_unwrap_rust_python_result(_kybra_scope.globals.get_item("get_message", vm), vm);
...
async fn set_message(_cdk_user_defined_new_message: String) -> (()) {
    unsafe {
        let _kybra_interpreter = _KYBRA_INTERPRETER_OPTION.as_mut().unwrap();
        let _kybra_scope = _KYBRA_SCOPE_OPTION.as_mut().unwrap();
        let vm = &_kybra_interpreter.vm;
        let method_py_object_ref =
            _kybra_unwrap_rust_python_result(_kybra_scope.globals.get_item("set_message", vm), vm);
        let invoke_result = vm.invoke(
            &method_py_object_ref,
            (_cdk_user_defined_new_message.try_into_vm_value(vm).unwrap(),),
        );
        match invoke_result {
            Ok(py_object_ref) => {
                let _kybra_final_return_value =
